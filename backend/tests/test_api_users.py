...
async def test_update_user_self(client, auth_headers, db: Session, test_user):
    ...
    response = await client.put(
        f"/api/users/{test_user.id}", json=update_data, headers=auth_headers
    )
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
...
async def test_update_user_other_forbidden(
    client, auth_headers, db: Session, test_user
):
    # Create another user
    other_user_data = {
        "name": "Other",
    ...
    response = await client.put(
        f"/api/users/{other_user_id}", json=update_data, headers=auth_headers
    )
