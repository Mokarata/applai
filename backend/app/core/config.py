...
class Settings(BaseSettings):
    ...
    ADMIN_NEW_PWD: Optional[str] = None
...
# Normalize SQLite DATABASE_URL to an absolute path to avoid CWD issues
try:
    if (
        ...
        logger.info(f"Using absolute SQLite DB path: {settings.DATABASE_URL}")
except Exception:
    logger.exception("Failed to normalize DATABASE_URL; continuing with provided value")
