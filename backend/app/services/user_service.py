...
from app.resources.prompts.cv_prompts import (
...
class UserService:
    def __init__(self, db: Session, llm_service: Optional[LLMServiceProtocol] = None):
        self.db = db
    ...
    def create_user(self, user_in: UserCreate) -> User:
        ...
        if self.get_user_by_email(user_in.email):
            logger.warning(
                "User creation failed: Email '%s' already registered.", user_in.email
            )
            raise HTTPException(
        ...
        if self.get_user_by_username(user_in.user_name):
            logger.warning(
                "User creation failed: Username '%s' already registered.",
                user_in.user_name,
            )
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        try:
            ...
            logger.info(
                "Successfully created user with ID: %s and email: %s",
                db_user.id,
                db_user.email,
            )
            return db_user
    ...
    def authenticate_user(self, email: str, password: str) -> Optional[User]:
        ...
        user = self.get_user_by_email(email) or self.get_user_by_username(email)
        if not user:
            return None
    ...
    def update_user(
        ...
        """
        logger.info("User %s attempting to update user %s.", current_user.id, user_id)
        ...
        try:
            ...
            if "is_admin" in update_data and not current_user.is_admin:
                logger.warning(
                    "Forbidden: User %s attempted to change admin status for user %s.",
                    current_user.id,
                    user_id,
                )
                raise HTTPException(
            ...
            if (
                ...
                if existing_user and existing_user.id != user_id:
                    logger.warning(
                        (
                            "User %s failed to update email for user %s: "
            ...
            self.db.rollback()
            logger.error(
                "Database error while updating user %s", user_id, exc_info=True
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    def delete_user(self, user_id: int) -> None:
        ...
        try:
            ...
            self.db.rollback()
            logger.error(
                "Database error while deleting user %s", user_id, exc_info=True
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    def _update_contact_info(self, db_user: User, new_contact_data: dict):
        ...
        if "address" in new_contact_data and isinstance(
            ...
            filtered_address = {
                k: v for k, v in new_address_data.items() if v != "string"
            }
    ...
    async def _extract_cv_data(self, cv_text: str) -> Optional[CVData]:
        ...
        try:
            ...
            logger.exception("Error extracting CV data")
            return None
    ...
    async def process_user_cv(
        ...
        logger.info(
            "Starting CV processing for user %s from file: %s",
            user_id,
            cv_file.filename,
        )
        try:
            ...
            if not processed_source or processed_source.metadata.get("error"):
                ...
                logger.warning(
                    "CV processing failed for user %s: %s", user_id, error_detail
                )
                raise HTTPException(
            ...
            logger.info("Extracting structured data from CV for user %s", user_id)
            structured_data = await self._extract_cv_data(
                processed_source.markdown_content
            )
            if structured_data:
                logger.info(
                    "Successfully extracted structured data for user %s", user_id
                )
                ...
                logger.warning(
                    "LLM extraction returned no structured data for user %s", user_id
                )
            ...
            logger.error(
                "An unexpected error occurred during CV processing for user %s",
                user_id,
                exc_info=True,
            )
            raise HTTPException(
