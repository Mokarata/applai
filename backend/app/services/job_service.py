...
from app.resources.prompts import JOB_EXTRACTION_SYSTEM, JOB_EXTRACTION_USER
...
class JobService:
    ...
    async def create_job_from_source(
        ...
        *,
        ...
        try:
            ...
            logger.error(
                f"Failed to process source for user {current_user.id}. Error: {e}",
                exc_info=True,
            )
            raise HTTPException(
    ...
    async def create_job(self, *, current_user: User, job_in: JobCreate) -> Job:
        ...
        try:
            ...
            self.db.rollback()
            logger.error(
                f"Database error creating job for user {current_user.id}: {e}",
                exc_info=True,
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def process_job(self, job_id: int) -> None:
        ...
        with next(get_db()) as db:
            try:
                ...
                # Create or fetch the company without scheduling another background task
                ...
                # In background context already: perform company analysis inline to avoid nested BackgroundTasks
                await company_service_bg.analyze_company(company_id=company.id)

                # Merge LLM data into job's extracted_data
                existing_extracted = job.extracted_data or {}
                new_extracted = extracted_data.model_dump(
