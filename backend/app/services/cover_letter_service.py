...
from app.schemas import (CoverLetterCreate, CoverLetterStructure,
                         ...
                         UserResponse)
from app.resources.prompts import cover_letter_prompts
...
class CoverLetterService:
    ...
    async def generate_cover_letter_instance(
        ...
        """
        logger.info(
            f"Generating cover letter instance for user_id: {user_id}, job_id: {job_id}"
        )
        try:
            ...
            generated_data: CoverLetterStructure = (
                await self.llm_service.generate_structured_output(
    ...
    def delete_cover_letters(self, cover_letter_ids: List[int], current_user: User):
        ...
        if len(letters_to_delete) != len(set(cover_letter_ids)):
            logger.warning(
                "Some cover letter IDs provided for deletion were not found."
            )
        ...
        try:
            ...
            self.db.commit()
            logger.info(
                f"Successfully deleted cover letters with IDs: {cover_letter_ids}"
            )
        except Exception as e:
            self.db.rollback()
            logger.error(
