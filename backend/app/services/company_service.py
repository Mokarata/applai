...
from app.resources.prompts import COMPANY_ANALYSIS_SYSTEM, COMPANY_ANALYSIS_USER
...
class CompanyService:
    ...
    def create_company(
        self, name: str, background_tasks: Optional[BackgroundTasks] = None
    ) -> Company:
        ...
        try:
            ...
            # Trigger background task to analyze the company, only if provided
            if background_tasks is not None:
                background_tasks.add_task(
                    self.analyze_company, company_id=db_company.id
                )
                logger.info(
                    "Enqueued analysis task for new company ID: %s", db_company.id
                )
    ...
    async def analyze_company(self, company_id: int) -> None:
        ...
        with next(get_db()) as db:
            ...
            try:
                ...
                if not company:
                    logger.error(
                        f"[Analyze_Company] Company ID '{company_id}' not found."
                    )
                    return

                logger.info(
                    f"Starting LLM analysis for company: {company.name} (ID: {company_id})"
                )
                ...
                if not analysis_result:
                    ...
                    logger.info(
                        f"Successfully received LLM analysis data for company: {company.name}"
                    )
