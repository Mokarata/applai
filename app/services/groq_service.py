...
class GroqService(LLMServiceProtocol):
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info(f"GroqService initialized successfully with model: {settings.GROQ_MODEL_NAME}")
    ...
    async def generate_text_from_prompt(
        ...
        if input_vars is None:
            input_vars = {}
        logger.info(f"Groq - Generating text output with input keys: {list(input_vars.keys())}")
        try:
    ...
    async def generate_structured_output(
        ...
        if input_vars is None:
            input_vars = {}
        logger.info(f"Groq - Generating Structured Output for schema: {output_schema.__name__}")
        try:
            ...
            system_message_prompt = SystemMessagePromptTemplate.from_template(full_system_prompt)
            human_message_prompt = HumanMessagePromptTemplate.from_template(user_prompt)
            chat_prompt = ChatPromptTemplate.from_messages([system_message_prompt, human_message_prompt])
            ...
            response = await chain.ainvoke(input_vars)
            logger.info("Groq - Successfully generated structured output.")
            return response
        except Exception as e:
            logger.error(f"Groq - Error generating structured output: {e}", exc_info=True)
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Error generating structured output from Groq: {e}"
            )
